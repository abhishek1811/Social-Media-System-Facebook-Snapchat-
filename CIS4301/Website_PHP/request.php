...
<body id="body">
  <div id="content">
      ...
      <script>
        <?php
          $query=pg_query($conn,"SELECT users.emailid, users.name, imgurl FROM images, users, mypic where users.emailid = images.emailid AND users.emailid=mypic.emailid AND images.imgid=mypic.imgid AND profile='t' AND users.emailid IN (Select friend1 from Request where friend2='$username');");
          while($row=pg_fetch_assoc($query))
          {
            $name_array[]=$row['name'];
            $email_array[]=$row['emailid'];
            $imgurl_array[]=$row['imgurl'];
          }
          $js_name_array = json_encode($name_array);
          $js_email_array = json_encode($email_array);
  ...
  <script type="text/javascript" src="showRequestCards.js"></script>
