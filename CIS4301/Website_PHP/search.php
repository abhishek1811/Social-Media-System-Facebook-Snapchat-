...
<body id="body">
	<div id="content">
        ...
        <template id="photo-template">
            <div class="template-border">
                <div class="image-holder">
                    <image class="question-mark"></image>
                </div>
        ...
        <!-- - - - - - - - - - - - - - - - - - - - - - - - - - -->
  		<script>
			<?php
				$search=$_GET['s'];
				$result=pg_query($conn,"SELECT users.emailid, users.name, imgurl FROM images, users, mypic where users.emailid = images.emailid AND users.emailid=mypic.emailid AND images.imgid=mypic.imgid AND profile='t' AND users.emailid IN (Select emailid from users where (name like '%$search%' or emailid like '%$search%')   and emailid !='{$_SESSION['name']}' and emailid!='admin' and emailid <> all (Select friend2 from Friends where Friend1='{$_SESSION['name']}')); ");
  ...
  <script type="text/javascript" src="showSearchCards.js"></script>
